...
class Coordinate():
    def __init__(self, x: float = 0, y: float = 0, z: float = 0, xyz = tuple[float, float, float]) -> None:
        self.x: float
        self.y: float
        self.z: float
        if y != None:
    ...
    def __add__(self, other):
        if other.__class__ == Coordinate:
            ...
            return Coordinate(abs(self.x)+abs(other.velocity.x), abs(self.y)+abs(other.velocity.y))
    def __repr__(self):
        return self.__str__()
    def __str__(self):
...
def TextInput(name: str, text: str, event: pygame.event.Event, color: pygame.Color, surface: pygame.Surface, pos: Coordinate):
