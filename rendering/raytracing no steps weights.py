...
numRaysPerPixel = 10
estimatedSecondsLow = numRaysPerPixel*11.36
print(time.gmtime(923784))
print(f'Estimated Time: {estimatedSecondsLow}-{numRaysPerPixel*18.53} Seconds ({round(estimatedSecondsLow/60, 2)}-{round(numRaysPerPixel*18.53/60, 2)} Minutes) End at: {time.strftime("%h%M%S", time.gmtime(time.time()+estimatedSecondsLow))}')
...
def RaySphere(ray: Ray, sphere: Sphere, seed: float, hitInfo: HitInfo):
    ...
    if discriminant >= 0:
        ...
        if dst >= 0:
            ...
            dsts.append(dst)
            depthModifier = 7
            #398.2113834851075/255 is min
            if hitInfo.color == backgroundColor:
                hitInfo = HitInfo(True, dst, hitPoint, numpy.array([x1/scalar, y1/scalar, z1/scalar]), (max(0, color[0]-(dst/depthModifier)), max(0, color[1]-(dst/depthModifier)), max(0, color[2]-(dst/depthModifier))))
            else:
                hitInfo = HitInfo(True, dst, hitPoint, numpy.array([x1/scalar, y1/scalar, z1/scalar]), utils.WeightedAverage([((max(0, color[0]-(dst/depthModifier)), max(0, color[1]-(dst/depthModifier)), max(0, color[2]-(dst/depthModifier))), 1), ((max(0, hitInfo.color[0]-(dst/depthModifier)), max(0, hitInfo.color[1]-(dst/depthModifier)), max(0, hitInfo.color[2]-(dst/depthModifier))), ray.bounces+1)]))
...
for x in range(screenSize[0]):
    for y in range(screenSize[1]):
        for rayNum in range(numRaysPerPixel):
            ...
            for object in objects:
                result = RaySphere(ray, object, Random(x+rayNum, y+rayNum), HitInfo(1,1,1,1,[0,0,0])).color
                if result != [0,0,0]:
                    #print(result)
