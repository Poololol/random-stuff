...
def background(f):
    def wrapped(*args, **kwargs):
        return asyncio.get_running_loop().run_in_executor(None, f, *args, **kwargs)
...
async def doray(x,y,maxRaySteps, numRaysPerPixel):
    intersecting = False
    i=0
    r=0
    g=0
...
for x in range(screenSize[0]):
    for y in range(screenSize[1]):
        pixels[x][y] = asyncio.run(doray(x,y,maxRaySteps,numRaysPerPixel))
t2 = time.time()
