...
rayPos = [0,0,0]
numRaysPerPixel = 2
print(f'Estimated Time: {numRaysPerPixel*11.26} Seconds when plugged in else: {numRaysPerPixel*16.73}')
maxRaySteps = 100
...
for x in range(screenSize[0]):
    for y in range(screenSize[1]):
        for rayNum in range(numRaysPerPixel):
            ...
            for object in objects:
                result = RaySphere(ray, object, Random(x, y), HitInfo(1,1,1,1,[0,0,0])).color
                if result != [0,0,0]:
                    #print(result)
