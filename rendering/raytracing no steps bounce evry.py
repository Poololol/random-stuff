...
class Sphere():
    def __init__(self, center, radius, color) -> None:
        ...
        self.color = color
class Ray():
    def __init__(self, origin: numpy.ndarray, numBounces: int, maxBounces: int, *, dir: numpy.ndarray = [], angles: list[float] = numpy.array([])) -> None:
...
for x in range(screenSize[0]):
    for y in range(screenSize[1]):
        for rayNum in range(numRaysPerPixel):
            ...
            hitInfo = HitInfo(1, 1, 1, numpy.array([0,0,1]), copy.copy(backgroundColor))
            while ray.bounces <= ray.maxBounces:
                for object in objects:
                    hitInfo = RaySphere(ray, object, Random(x, y), hitInfo)
                if hitInfo.didHit == True:
                        ray.dir = CalcBounceDir(ray, object, hitInfo, Random(x, y))
                        ray.bounces += 1
                else:
                    break
