    ...
    if discriminant >= 0:
        ...
        if dst >= 0:
            ...
            if hitInfo.color == backgroundColor:
                hitInfo = HitInfo(True, dst, hitPoint, numpy.array([x1/scalar, y1/scalar, z1/scalar]), (max(0, color[0]-(dst/2)), max(0, color[1]-(dst/2)), max(0, color[2]-(dst/2))))
            else:
                hitInfo = HitInfo(True, dst, hitPoint, numpy.array([x1/scalar, y1/scalar, z1/scalar]), utils.WeightedAverage([((max(0, color[0]-(dst/2)), max(0, color[1]-(dst/2)), max(0, color[2]-(dst/2))), 1), ((max(0, hitInfo.color[0]-(dst/2)), max(0, hitInfo.color[1]-(dst/2)), max(0, hitInfo.color[2]-(dst/2))), ray.bounces*2)]))
