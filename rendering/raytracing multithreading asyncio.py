...
pri = False
@background
def doray(x,y,maxRaySteps, numRaysPerPixel):
    intersecting = False
    i=0
...
looper = asyncio.gather(*[doray(x,y, maxRaySteps, numRaysPerPixel) for x, y in itertools.product(range(int(screenSize[0]/2)), range(int(screenSize[1]/2)))])
