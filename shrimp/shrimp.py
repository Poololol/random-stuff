...
colors:list[tuple] = [utils.red, utils.pink, utils.yellow, utils.blue, utils.orange, utils.green, utils.purple]
players:dict[list] = {}
menu = True
optionNum = 3
optionNum2 = 4
...
frame = 0
offset = 0,0
cardWidth = screenSize[0]/25 if screenSize[0] < 750 else screenSize[0]/50
cardHeight = int((cardWidth*2)/3)*3
...
while True:
    ...
    smallFont = pygame.font.Font(None, int(screenSize[1]/22))
    mousePos = pygame.mouse.get_pos()
    mouseRect = pygame.Rect(mousePos, (1, 1))
    pygame.draw.rect(screen, backgroundColor, ((0, 0), screenSize), width=0)
    cardWidth = screenSize[0]/25 if screenSize[0] < 750 else screenSize[0]/50
    cardHeight = int((cardWidth*2)/3)*3
    buttonSize = cardWidth*2
    random.seed(seed)
    b = random.randint(-3,-1)
    pygame.draw.rect(screen, utils.white, ((screenSize[0]/2)-(cardWidth/2)-borderSize, (screenSize[1]/2)-(cardHeight/2)-borderSize, cardWidth+(borderSize*2), cardHeight+(borderSize*2)), width=0)
    pygame.draw.rect(screen, deck[a][b], ((screenSize[0]/2)-(cardWidth/2), int((screenSize[1]/2)-(cardHeight/2)+((cardHeight/3)*0)), cardWidth, cardHeight/3))
    pygame.draw.rect(screen, deck[a][b+1], ((screenSize[0]/2)-(cardWidth/2), int((screenSize[1]/2)-(cardHeight/2)+((cardHeight/3)*1)), cardWidth, cardHeight/3))
    pygame.draw.rect(screen, deck[a][b+2], ((screenSize[0]/2)-(cardWidth/2), int((screenSize[1]/2)-(cardHeight/2)+((cardHeight/3)*2)), cardWidth, cardHeight/3))
    i = 0
    ...
    for player in players.items():
        ...
        for card in newlist:
            pygame.draw.rect(cards, utils.white, ((screenSize[0]/2)-(cardWidth/2)+((j-((len(newlist)-1)/2))*(cardSpacing+cardWidth))-borderSize, screenSize[1]-(cardHeight*2)-borderSize, cardWidth+borderSize*2, cardHeight+borderSize*2))
            pygame.draw.rect(cards, card[0], ((screenSize[0]/2)-(cardWidth/2)+((j-((len(newlist)-1)/2))*(cardSpacing+cardWidth)), screenSize[1]-(cardHeight*2), cardWidth, cardHeight))
